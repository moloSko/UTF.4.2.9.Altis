...
switch _mode do
{
	...
	case "Init": 
	{
		...
		["TB_fnc_arsenal"] call bis_fnc_startloadingscreen;
		...
		//INITTYPES 
		["InitGUI", [_display, "TB_fnc_arsenal"]] call bis_fnc_arsenal;
		...
		["TB_fnc_arsenal"] call bis_fnc_endloadingscreen;
	};
	...
	case "InitGUI": 
	{
		...
		if (is3DEN) then 
		{
			private _isArsenal = _function == "TB_fnc_arsenal";
		...
		_ctrlButtonImport ctrlenable !ismultiplayer; // cannot copy from clipboard in MP, only to clipboard
		...
		_ctrlTemplateButtonDelete ctrladdeventhandler ["buttonclick","with uinamespace do {['buttonTemplateDelete',[ctrlparent (_this select 0)],true] call bis_fnc_arsenal;};"];
		...
		BIS_fnc_arsenal_draw3D = addMissionEventHandler ["draw3D", { with uiNamespace do { ["draw3D"] call bis_fnc_arsenal } }];
	...
	case "Preload": 
	{
		...
		if (isnil "_data") then 
		{
			...
			
			{
				...
				private _scope = if (isnumber (_x >> "scopeArsenal")) then {getnumber (_x >> "scopeArsenal")} else {getnumber (_x >> "scope")};
				...
				then 
				{
					...
					{						
						...
						if (_typesDataIndex >= 0) then { _data select _typesDataIndex pushBack configname _x };
					};
				};
			...
			if (!is3DEN) then 
			{
				...
				{	
					{
						if (gettext (_x >> "head") != "" && configname _x != "Default") then
						{
							_faces select 0 pushback configname _x;
							_faces select 1 pushback _x;
							if (getnumber (_x >> "disabled") > 0) exitWith {}; 	
							_data select IDC_RSCDISPLAYARSENAL_TAB_FACE pushback configName _x;
						};
					} 
				...
				//--- Voices
				{
					...
					{
						_data select IDC_RSCDISPLAYARSENAL_TAB_VOICE pushBack configname _x;
					};
				} 
				...
				//--- Insignia
				{
					...
					if (_scope == 2) then 
					{
						_items = _data select IDC_RSCDISPLAYARSENAL_TAB_INSIGNIA pushBack configname _x;
					};
				} 
			...
			
			{
				...
				{
					{
						if (_magazines pushBackUnique _x > -1) then
						{
							private _cfgMag = _CFGMAGAZINES >> _x;
							if (getnumber (_cfgMag >> "scope") == 2 || getnumber (_cfgMag >> "scopeArsenal") == 2) then 
							{
								_data select _tab pushBack configname _cfgMag;
								_magazinesThrowPut pushBack tolower _x;
							};
						};
					}
			...
			//private _ctrlList = _display displayctrl (IDC_RSCDISPLAYARSENAL_LIST + IDC_RSCDISPLAYARSENAL_TAB_CARGOMAGALL);
			{
				...
				{
					_data select IDC_RSCDISPLAYARSENAL_TAB_CARGOMAGALL pushBack configname _x;
				};
			} 
	...
	case "ListSelectCurrent": 
	{
		...
		
		{
			...
			if (_select) then 
			{
				...
				if (_selectIndex > -1) then { _ctrlList lbsetcursel _selectIndex };
			};
	...
	case "buttonTemplateDelete": 
	{
		...
		if (_confirmAction isEqualTo true) exitWith
		{
			_this spawn 
			{
				...
				{ 
					isNil { with uinamespace do { ["buttonTemplateDelete", _this, !_confirmAction] call bis_fnc_arsenal } };
				};
			};
		};
		...
		['showTemplates',[_display]] call (uinamespace getvariable (["TB_fnc_arsenal","bis_fnc_garage"] select BIS_fnc_arsenal_type));
...
*/
	...
	case "showTemplates": 
	{
		...
		for "_i" from 0 to (count _data - 1) step 2 do {
			...
			if (
				{_item = _x; !CONDITION(_virtualWeaponCargo) || !isclass(_CFGWEAPONS >> _item)} count _inventoryWeapons > 0
				||
				...
				||
				{_item = _x; !CONDITION(_virtualBackpackCargo) || !isclass(_CFGVEHICLES >> _item)} count _inventoryBackpacks > 0
			) then {
	...
	case "buttonExport":
	{
		...
		["showMessage", [_this select 0, localize "STR_a3_RscDisplayArsenal_message_clipboard"]] call bis_fnc_arsenal;
	};
...
*/
	...
	case "buttonSave": {
		_display = _this select 0;
		['showTemplates',[_display]] call (uinamespace getvariable (["TB_fnc_arsenal","bis_fnc_garage"] select BIS_fnc_arsenal_type));//bis_fnc_arsenal;
	...
	case "buttonRandom": 
	{
		...
		//--- Right sidebar (attachments)
		{
			...
			{
				private _acc = selectRandom (getarray (_x >> "compatibleItems") + [""]);
				if (_acc != "") then 
				{
					switch _weaponID do 
					{
	...
	case "buttonSpace": {
		...
		_function = ["TB_fnc_arsenal","bis_fnc_garage"] select _buttonID;
	...
	case "AmmoboxInit": 
	{
		...
		[["AmmoboxServer",_box,true],"TB_fnc_arsenal",false] call bis_fnc_mp;
	};
	...
	case "AmmoboxExit": 
	{
		...
		[["AmmoboxServer",_box,false],"TB_fnc_arsenal",false] call bis_fnc_mp;
	};
	...
	case "AmmoboxServer": 
	{
		...
		["AmmoboxLocal","TB_fnc_arsenal",true,isnil "bis_fnc_arsenal_ammoboxServer"] call bis_fnc_mp;
	...
	case "AmmoboxLocal": 
	{
		_boxes = missionnamespace getvariable ["bis_fnc_arsenal_boxes",[]];
		{
			if (isnil {_x getvariable "bis_fnc_arsenal_action"}) then {
				...
				//		//["Open",[nil,_box,_unit]] call TB_fnc_arsenal;
				//	},
				//	[],
				//	0,
				//	false,
