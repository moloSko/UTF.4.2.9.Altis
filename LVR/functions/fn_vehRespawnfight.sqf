...
if ((typeOf _veh) isEqualTo "B_T_UAV_03_dynamicLoadout_F" || (typeOf _veh) isEqualTo "O_T_UGV_01_rcws_ghex_F") then {
...
while { true } Do {
	...
	if (_abandonDelay !=0 && ( alive _veh ) /*&& { ( canMove _veh )*/ && { { ( alive _x ) } count ( crew _veh ) == 0 } ) then {
		...
		{
			if (_x distance2D _veh < 2000) exitWith {_abandoned = false;};
