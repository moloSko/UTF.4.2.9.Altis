...
while { true } Do {
	...
	if (_abandonDelay !=0 && ( alive _veh ) /*&& { ( canMove _veh )*/ && { { ( alive _x ) } count ( crew _veh ) == 0 } ) then {
		...
		{
			if (_x distance2D _veh < 10) exitWith {_abandoned = false;};
		...
		if ( ( _abandoned ) && { _veh distance2D _vehPos > 10 } && { _unit distance2D _veh > 10 } ) then {
			deleteVehicle _veh;
			sleep 1;
			...
			if (typeOf _veh in ["O_UAV_02_dynamicLoadout_F","O_UGV_01_rcws_F","O_T_UGV_01_rcws_ghex_F"]) then {
